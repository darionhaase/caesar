...
#RUN ln -s "/usr/bin/aarch64-alpine-linux-musl-g++" "/usr/bin/musl-g++"

# install required dependencies to build z3.rs...
#RUN apk add cmake llvm-dev libclang-dev clang
RUN apk add cmake make gcc libtool musl-dev g++
RUN apk add build-base alpine-sdk make cmake clang llvm clang-dev git
